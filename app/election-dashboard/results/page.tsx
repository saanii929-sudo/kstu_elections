    ...
    <div className="max-w-7xl">
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-lg text-gray-400 mt-0.5">Last updated {lastUpdated.toLocaleTimeString()}</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <RefreshCw size={16} className={autoRefresh && refreshing ? 'animate-spin' : ''} />
      ...
      {/* Election Selector */}
      <div className="mb-6">
        <label className="block text-lg font-medium text-gray-700 mb-1.5">Election</label>
        <select
        ...
        <>
          ...
          {electionEnded && (
            <div className="bg-white border border-gray-200 rounded-xl p-4 mb-6 flex items-center gap-3">
              ...
              <div>
                <p className="font-bold text-gray-900 text-lg">Election concluded</p>
          ...
          <div className="flex items-center gap-3 mb-6">
            <h2 className="text-lg font-bold text-gray-900 whitespace-nowrap">Results by Position</h2>
            <div className="flex-1 h-px bg-gray-100" />
            <span className="text-lg text-gray-400 whitespace-nowrap">{Object.keys(groupedCandidates).length} position{Object.keys(groupedCandidates).length !== 1 ? 's' : ''}</span>
          </div>
          ...
          {loading ? (
            ...
            <div className="bg-white border border-gray-100 rounded-2xl p-16 text-center">
              ...
              <p className="text-lg text-gray-500">Results will appear once voting begins</p>
            </div>
          ) : viewMode === 'grid' ? (
                    ...
                    <div key={positionName} className="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm">
                      <div className="px-6 py-4 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <h3 className="text-2xl font-bold text-black">{positionName}</h3>
                        </div>
                      ...
                      <div className="p-6">
                        ...
                        <div className="flex items-center gap-4 mb-6 pb-5 border-b border-gray-100">
                          {solo.image ? (
                            <img src={solo.image} alt={solo.name} className="w-44 h-44 rounded-lg object-cover border-2 border-green-100 shadow-sm shrink-0" />
                          ) : (
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-bold text-2xl text-gray-900 truncate">{solo.name}</p>
                            <p className="text-lg text-gray-500 mt-0.5">Running for <span className="font-semibold text-gray-700">{positionName}</span></p>
                        ...
                        {positionCandidates.map((candidate, i) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {/* Photo */}
                              {candidate.image ? (
                                <img src={candidate.image} alt={candidate.name} className="w-44 h-44 rounded-lg object-cover border-2 border-white shadow-sm shrink-0" />
                              ) : (
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-4">
                                  <p className="font-bold text-gray-900 text-xl truncate">{candidate.name}</p>
                                  {isTiedCandidate && (
                                  ...
                                  {isWinner && (
                                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-lg font-bold shrink-0 ${electionEnded ? 'bg-[#d4af37] text-white' : 'bg-green-100 text-[#d4af37]'}`}>
                                      <Award size={14} />
                              ...
                              {/* Vote count */}
                              <div className="text-right shrink-0">
                                <p className="text-3xl font-bold text-gray-900">{candidate.voteCount.toLocaleString()}</p>
                                <p className="text-xl font-semibold text-gray-400">{pct}%</p>
                              </div>
                            </div>
                          );
                        })}
                          ...
                          <tbody>
                            ...
                            <tr className={`border-b border-gray-100 ${isSoloTied ? 'bg-amber-50' : 'bg-green-50'}`}>
                              <td className="py-4 px-6">
                                <div className="flex items-center gap-3">
                                  {solo.image ? (
                                    <img src={solo.image} alt={solo.name} className="w-22 h-22 rounded-lg object-cover border-2 border-green-100 shadow-sm shrink-0" />
                                  ) : (
                        ...
                        return (
                          <div key={candidate._id} className={`p-4 ${isTiedCandidate ? 'bg-amber-50/40' : isWinner ? 'bg-green-50' : ''}`}>
                            <div className="flex items-center gap-3 mb-2.5">
                              ...
                              {candidate.image ? (
                                <img src={candidate.image} alt={candidate.name} className="w-22 h-22 rounded-lg object-cover border-2 border-white shadow-sm shrink-0" />
                              ) : (
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="font-semibold text-lg text-gray-900 truncate">{candidate.name}</p>
                                <p className="text-sm text-gray-500">{candidate.voteCount.toLocaleString()} votes · {pct}%</p>
                              </div>
                              {isTiedCandidate ? (
                                <span className="text-lg font-bold px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 shrink-0">Tied</span>
                              ) : isWinner ? (
                                <span className={`text-sm font-bold px-2 py-0.5 rounded-full shrink-0 ${electionEnded ? 'bg-[#d4af37] text-white' : 'bg-green-100 text-[#d4af37]'}`}>
                                  {electionEnded ? 'Elected' : 'Leading'}
                                </span>
                              ) : (
                          ...
                          {positionCandidates.map((candidate, i) => {
                            ...
                            return (
                              <tr key={candidate._id} className={`transition-colors
                                ...
                                <td className="py-4 px-6 text-right">
                                  <p className="text-xl font-bold text-gray-900">{candidate.voteCount.toLocaleString()}</p>
                                </td>
                                <td className="py-4 px-6">
                                  <div className="flex items-center gap-2">
                                    <div className="flex-1 bg-gray-100 rounded-full h-4 overflow-hidden">
                                    ...
                                    <span className="text-lg font-semibold text-gray-600 w-10 text-right">{pct}%</span>
                                  </div>
                                </td>
                                <td className="py-4 px-6">
                                  {isTiedCandidate ? (
                                    ...
                                    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-sm font-bold ${electionEnded ? 'bg-[#d4af37] text-white' : 'bg-green-100 text-[#d4af37]'}`}>
                                      <Award size={11} />
                                      {electionEnded ? 'Elected' : 'Leading'}
                                    </span>
                                  ) : (
